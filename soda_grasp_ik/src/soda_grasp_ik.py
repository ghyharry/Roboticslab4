...
def main(if_sim):
    ...
    for configuration in configurations:
        ...
        adaRRT = AdaRRT(start_state=np.array(arm_home),
    ...
    for _ in range(125):
        ...
        delta_x = [0, 0, 0, -0.004, 0, 0]
        current_J = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
        temp = np.matmul(current_J, current_J.T)
        temp = np.linalg.pinv(temp)
        temp = np.matmul(current_J.T, temp)
        q_error = np.matmul(temp, delta_x)
        q = q - q_error
        ada.set_positions(q)
